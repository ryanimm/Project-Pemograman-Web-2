		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="nama_berita" placeholder="Nama Berita ..." required>
				</div>	
				...
				<div class="form-group">
					...
					<textarea class="form-control" style="white-space: pre-line;" rows="5" name="keterangan_berita" placeholder="Keterangan Berita ..." required></textarea>
				</div>										
			</div>
...
<?php 
	$g = mysqli_query($conn,"SELECT * FROM tbl_berita");
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="form-group">
					<label>Nama Berita</label>
					<input type="text" class="form-control" value="<?php echo $d['nama_berita'] ?>" name="nama_berita" placeholder="Nama Berita ..." required>
				</div>	

				<div class="form-group">
					<img src="../img/Berita/<?php echo $d['gambar_berita'] ?>" width="70" height="70">
				</div>	
				...
				<div class="form-group">
					<label>Keterangan Berita</label>
					<textarea class="form-control" style="white-space: pre-line;" rows="5" name="keterangan_berita" placeholder="Keterangan Berita ..." required><?php echo $d['keterangan_berita'] ?></textarea>
				</div>										
			</div>
...
<?php 
	$h = mysqli_query($conn,"SELECT * FROM tbl_berita");
...
<?php 
	...
	if(isset($_POST['simpan']))
	{
		...
		if($gambar_berita != "") {
		   ...
		   if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {     
		      move_uploaded_file($file_tmp, '../img/Berita/'.$nama_gambar_baru);
		      
		      $query  = "INSERT INTO tbl_berita (nama_berita, keterangan_berita, keterangan_gambar, gambar_berita) 
		      ...
		      $result = mysqli_query($conn, $query);
		                ...
		                if(!$result){
		                    ...
		                    echo "<script>alert('Data berhasil ditambah.');window.location='?view=berita';</script>";
		                }
		...
		$keterangan_berita = mysqli_real_escape_string($conn,$_POST['keterangan_berita']);
	  	...
	  	if($gambar_berita != "") {
	       ...
	       if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
	       	  ...
	       	  $get_foto  = "SELECT gambar_berita FROM tbl_berita WHERE id='$id'";
		      $data_foto = mysqli_query($conn, $get_foto); 
		      $foto_lama = mysqli_fetch_array($data_foto);

		      unlink("../img/Berita/".$foto_lama['gambar_berita']); 

	          move_uploaded_file($file_tmp, '../img/Berita/'.$nama_gambar_baru);
	                      
	          $query  = "UPDATE tbl_berita SET nama_berita = '$nama_berita', keterangan_berita = '$keterangan_berita', keterangan_gambar = '$keterangan_gambar', 
	          ...
	          $result = mysqli_query($conn, $query);
	                    ...
	                    if(!$result){
	                      ...
	                      echo "<script>alert('Data berhasil diubah.');window.location='?view=berita';</script>";
	                    }
	       } else {     
	          echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png atau jpeg.');window.location='?view=berita';</script>";
	       }
	...
	elseif(isset($_POST['hapus']))
	{
	    ...
	    if(!$hasil_query) {
	      ...
	      echo "<script>alert('Data berhasil dihapus.');window.location='?view=berita';</script>";
	    }
	}
?>